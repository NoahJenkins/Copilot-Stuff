...
#### 3C — OpenAI Codex / GitHub Copilot Coding Agent

**Global instructions → `AGENTS.md`**

`AGENTS.md` at the repository root is an open cross-tool standard. It is read by **OpenAI Codex**, **GitHub Copilot's coding agent** (added August 2025), **OpenCode**, and other AI coding agents. If neither Codex nor OpenCode is detected but the file already exists (e.g., created by the onboard prompt for Copilot coding agent compatibility), still write to it.
...
> To propagate changes, update `.github/copilot-instructions.md` and re-run that command.
>
> Read by: GitHub Copilot coding agent, OpenAI Codex, OpenCode, Cursor, Gemini CLI, and other
> AI coding tools that support the AGENTS.md cross-tool standard.
...
OpenCode **reuses `AGENTS.md`** at the repo root (same as Codex and GitHub Copilot's coding agent). If `AGENTS.md` was already
written in Step 3C, OpenCode will automatically pick it up — no additional file needed.

If OpenCode is detected but neither Codex nor an existing `AGENTS.md` is present, write `AGENTS.md` using the same format as Step 3C.
...
| GitHub Copilot (coding agent) | `AGENTS.md` (shared)   | Markdown               | Full copy           |
