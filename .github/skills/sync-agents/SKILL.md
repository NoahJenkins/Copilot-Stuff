...
- **Nested `AGENTS.md` files** — any `AGENTS.md` file found outside the repo root and outside `.github/`. For each, record its relative directory path (e.g., `docs/`, `src/`) and full content. These are directory-scoped sources of truth and will be synced to matching per-directory tool config files in Step 3.
...
**Important:** When inserting the content of `AGENTS.md`, ensure you completely overwrite the existing file content. Do not append the content to the existing file, as this will cause duplication.
...
**Nested `AGENTS.md` → `<dir>/CLAUDE.md`**

For each nested `AGENTS.md` found in Step 1 (e.g., `docs/AGENTS.md`), create a matching
`CLAUDE.md` in the same directory. Claude Code reads `CLAUDE.md` files at each directory
level, so this provides directory-scoped context when working inside that subtree.

```markdown
<!--
  ╔══════════════════════════════════════════════════════════════╗
  ║  AUTO-SYNCED — DO NOT EDIT THIS FILE DIRECTLY               ║
  ║  Source of truth: <dir>/AGENTS.md                           ║
  ║  To update: run /sync-agents                                ║
  ╚══════════════════════════════════════════════════════════════╝
-->
...
[INSERT FULL CONTENT OF <dir>/AGENTS.md HERE]
```
...
**Nested `AGENTS.md` → `<dir>/GEMINI.md`**

For each nested `AGENTS.md` found in Step 1, create a matching `GEMINI.md` in the same
directory using Gemini's native `@import` syntax:
...
**Important:** Do **not** overwrite any nested `AGENTS.md` that was found in Step 1 as a
hand-authored source. If a nested `AGENTS.md` already exists in a target directory, it is
the source of truth for that directory — skip creating or overwriting it here.
...
📂 Nested AGENTS.md found: <list of relative paths, or "none">
...
Files written:
  ...
  Nested directory syncs:
    - <dir>/CLAUDE.md                          (synced from <dir>/AGENTS.md)
    - <dir>/GEMINI.md                          (synced from <dir>/AGENTS.md)
    ...
...
| Claude Code     | `<dir>/CLAUDE.md`                 | Markdown               | Full copy (per nested `AGENTS.md`) |
| Gemini CLI      | `GEMINI.md`                       | Markdown + `@imports`  | Live import         |
| Gemini CLI      | `<dir>/GEMINI.md`                 | Markdown + `@imports`  | Live import (per nested `AGENTS.md`) |
| OpenAI Codex    | `AGENTS.md` (read natively)       | Markdown               | **Source of truth** (no sync needed) |
| OpenAI Codex    | `<dir>/AGENTS.md` (read natively) | Markdown               | **Source of truth** (no sync needed) |
